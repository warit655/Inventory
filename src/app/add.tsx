...
export default function AddScreen() {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/products`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
      if (res.ok) {
        showAlert('เพิ่มสินค้าสำเร็จ', `นำ ${formData.name} เข้าสู่ระบบแล้ว`, () => router.back());
      } else {
        showAlert('ข้อผิดพลาด', 'บันทึกไม่สำเร็จ');
      }
    } catch (err) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          <View style={styles.formCard}>
            ...
            <TextInput style={styles.input} placeholder="เช่น RTX 5090" placeholderTextColor="#9CA3AF" value={formData.name} onChangeText={t => setFormData({...formData, name: t})} />

            <View style={styles.row}>
              <View style={styles.half}><Text style={styles.label}>แบรนด์</Text><TextInput style={styles.input} placeholder="ASUS" placeholderTextColor="#9CA3AF" value={formData.brand} onChangeText={t => setFormData({...formData, brand: t})} /></View>
              <View style={styles.half}><Text style={styles.label}>หน่วยความจำ (VRAM)</Text><TextInput style={styles.input} placeholder="32GB" placeholderTextColor="#9CA3AF" value={formData.vram} onChangeText={t => setFormData({...formData, vram: t})} /></View>
            </View>

            <Text style={styles.label}>ซีเรียลนัมเบอร์ (S/N)</Text>
            <TextInput style={styles.input} placeholder="สแกนหรือพิมพ์ S/N" placeholderTextColor="#9CA3AF" value={formData.serial_number} onChangeText={t => setFormData({...formData, serial_number: t})} />

            <View style={styles.row}>
              <View style={styles.half}><Text style={styles.label}>ราคาต้นทุน</Text><TextInput style={styles.input} placeholder="0.00" placeholderTextColor="#9CA3AF" keyboardType="numeric" value={formData.cost_price} onChangeText={t => setFormData({...formData, cost_price: t})} /></View>
              <View style={styles.half}><Text style={styles.label}>ราคาขาย</Text><TextInput style={styles.input} placeholder="0.00" placeholderTextColor="#9CA3AF" keyboardType="numeric" value={formData.selling_price} onChangeText={t => setFormData({...formData, selling_price: t})} /></View>
            </View>

            <View style={styles.row}>
              <View style={styles.half}><Text style={styles.label}>จำนวน (สต็อก)</Text><TextInput style={styles.input} placeholder="0" placeholderTextColor="#9CA3AF" keyboardType="numeric" value={formData.stock} onChangeText={t => setFormData({...formData, stock: t})} /></View>
              <View style={styles.half}><Text style={styles.label}>หมวดหมู่</Text><TextInput style={styles.input} placeholder="GPU" placeholderTextColor="#9CA3AF" value={formData.category} onChangeText={t => setFormData({...formData, category: t})} /></View>
            </View>

            <Text style={styles.label}>ลิงก์รูปภาพ (URL)</Text>
            <TextInput style={styles.input} placeholder="https://..." placeholderTextColor="#9CA3AF" value={formData.image} onChangeText={t => setFormData({...formData, image: t})} />
          </View>
        </ScrollView>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={[styles.saveBtn, loading && { opacity: 0.7 }]} onPress={handleSave} disabled={loading}>
          <Text style={styles.saveBtnText}>{loading ? 'กำลังบันทึก...' : 'บันทึกข้อมูลสินค้า'}</Text>
        </TouchableOpacity>
      </View>
